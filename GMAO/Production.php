...
if ($id !== ""){
    ...
    $sql1="SELECT * FROM production WHERE user_id=:user_id";
    $statement = $connection->prepare($sql1);
    $statement->bindValue(":user_id",$id);
    $statement->execute();
    $resultproduction = $statement->fetchAll();
    foreach($resultproduction as $row){
        $id_production=$row["id"];
    }
    $sql2 ="SELECT * FROM production JOIN formulaire WHERE (production.user_id =:user_id) AND( formulaire.type_formulaire =:type_formulaire) AND (formulaire.production_id=:production_id);";
    $statement = $connection->prepare($sql2);
    $statement->bindValue(":type_formulaire","Maintenance");
    $statement->bindValue(":user_id",$id);
    $statement->bindValue(":production_id",$id_production);
    $statement->execute();
...
<body>
    <div class="header" id="header">
        <div class="container">
            ...
            <div class="links">
            <ul class="link">
                <li><a href="Production.php" class="active">Request For Intervention</a></li>
            </ul>
            <ul class="link_2">
    ...
    <table id="requests-table">
        ...
        <div class="info">
            <div class="info-2">
                ...
                <input type="datetime-local" value="<?php echo $row["date_agent"]?>" disabled>
            </div>
        </div>
        <div class="info">
            <div class="info-2">
...
<?php endif; ?>
        ...
        <div class="box">
            ...
            <div class="info">
                <div class="info-2">
                    <label >DATE/HOUR(Maintenance):</label>
                    <input  name="date_maintenance" type="datetime-local" value="<?php echo $row["date_maintenance"]?>"disabled >
                </div>
            </div>
        </div>
        <div class="box">
            <div class="info">
                ...
                <input type="text" name="name_production" class="name" value="<?php echo $row["first_name"]." ".$row["last_name"] ?>">
                </div>
               </div>
            <div class="info">
                <div class="info-2">
                    <label >DATE/HOUR(Production):</label>
                    <input id="date" name="date_production" type="datetime-local" >
                </div>
            </div>
            <!-- <div class="info">
